...
// function emailPush(list, email) {
//     list.push(email);
// }
// const result1 = emailPush("student", "test@test.nl")
// console.log(result1);
...
const checkNick = emailadresses.includes("n.stuivenberg@novi.nl");
...
function findMailByLastName(list,lastname) {
    let result = null;
...
const findNova = findMailByLastName(emailadresses, "wiersma");
console.log(findNova);
...
function inserBefore(list, emailY, emailZ){
    const indexZ = list.indexOf(emailZ);
    list.splice(indexZ, 0, emailY);
}
...
function checkAdres(arrayEmailAdres, oldMail, newMail) {
    const indexofZ = arrayEmailAdres.indexOf(oldMail);
    arrayEmailAdres[indexofZ] = newMail;
}
const testResult = checkAdres(emailadresses,"m.vandergeest@novi-education.nl", "m.vandergeest@novi.nl");
console.log(emailadresses);